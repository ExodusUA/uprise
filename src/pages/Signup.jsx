...
function Signup() {
    ...
    function validateForm() {
        if (name.length === 0) {
            setValidateError('You did not enter your First Name!')
        } else if (lastName.length === 0) {
            setValidateError('You did not enter your Last Name!')
        } else if (!email.match(regex)) {
            setValidateError('Incorrect email entered!')
        } else if (pass.length < 8) {
            setValidateError('Incorrect password entered!')
        } else if (checkbox === false) {
            setValidateError('Accept the UpRise Terms and Conditions and agree to the Privacy Policy!')
        }
        else {
            setValidateError('')
            fetch('https://backend.uprise.partnerswire.com/v1/user/create', options)
                .then(response => response.json())
                .then(response => {
                    authUser()
                    if (response.length === 0) {
                        setValidateError('User successfully created.')
    ...
    function authUser() {
        ...
        console.log(AuthOptions.body)
        fetch('https://backend.uprise.partnerswire.com/v1/user/login/password', AuthOptions)
            .then(response => response.json())
            .then(response => {
    ...
    return (
            <section className='authWrapper'>
                <div className='authContent'>
                    ...
                    <div className='authContentForm signupAuth'>
                        <p className='authFormTitle'>Create an account</p>
                        <p className="authFormSubtitle">Let’s start your journey!</p>


                        <AuthInput placeholder='First Name' type="name" value={name} updateValue={setName} />
                        <AuthInput placeholder='Last Name' type="name" value={lastName} updateValue={setLastName} />
                        <AuthInput placeholder='Email' type="email" value={email} updateValue={setEmail} />
                        <AuthInput placeholder='Password' type="password" value={pass} updateValue={setPass} />
                        ...
                        <div className="authFormButtons">
                            <div className='signupCheckbox'>
                                ...
                                <label htmlFor="authCheckbox">By registering my account, I accept UpRise Terms & Conditions and agree with the Privacy Policy</label>
                            </div>
                        </div>
                        <p className='authError'>{validateError}</p>
                        <AuthButton text="Create account" validateForm={validateForm} />
